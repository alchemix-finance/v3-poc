...
contract TransmuterBuffer is ITransmuterBuffer, AccessControl, Initializable {
    ...
    /// @inheritdoc ITransmuterBuffer
    function setWeights(address weightToken, address[] memory tokens, uint256[] memory weights) external override onlyAdmin {
    ...
    /// @inheritdoc ITransmuterBuffer
    function withdraw(address underlyingToken, uint256 amount, address recipient) external override onlyTransmuter(underlyingToken) {
    ...
    function burnCredit() external override onlyKeeper {
        ...
        IAlchemistV3(alchemist).mint(credit, address(this));
